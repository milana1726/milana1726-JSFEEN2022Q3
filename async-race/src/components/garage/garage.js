...
import {
   addArrowsSort, removeDisabled, setDisabled, setStartCount
} from './garage-options';
import {
...
export default class Garage {
    ...
    renderCars(cars) {
        cars.forEach((el) => new Car(el));
    }

    renderWinners(winners) {
        const tableWinners = new Winner(winners);
    ...
    async updateStateWinners() {
        const winners = await getWinners({page: this.pageWinners, sort: store.sortBy, order: store.order});
